...
@implementation NSData (DKEncodeAndDecode)

- (NSString *)dk_utf8String {
    if (self.length > 0) {
...
+ (NSData *)dk_dataWithHexString:(NSString *)hexStr {
...
+ (NSData *)dk_dataWithBase64EncodedString:(NSString *)base64EncodedString {
...
- (id)dk_jsonValueDecoded {
    NSError *error = nil;
    id value = [NSJSONSerialization JSONObjectWithData:self options:kNilOptions error:&error];
    if (error) {
        DKLogError(@"jsonValueDecoded error:%@", error);
    }
    return value;
}
