...
@implementation NSArray (DKBlock)
...
{
    return [self dk_reduce:nil withBlock:block];
}
...
{
    if (!block) return kDKArrayErrorIndexValue;
    
    for (NSInteger i = 0; i < self.count; i++) {
        id object = [self objectAtIndex:i];
        if (block(object)) return i;
    }
    
    return kDKArrayErrorIndexValue;
}
